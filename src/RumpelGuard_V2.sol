...
import {RumpelTimelockRegistry} from "./RumpelTimelockRegistry.sol";

/// @notice Rumpel Safe Guard V2 with timelock support for the Rumpel Wallet.
/// @dev Compatible with Safe v1.3.0-libs.0, the last Safe Ethereum mainnet release, so it can't use module execution hooks.
contract RumpelGuard_V2 is Ownable, IGuard {
    ...
    address public timelockRegistry; // Optional timelock registry
    ...
    event TimelockRegistrySet(address indexed registry);

    error CallNotAllowed(address target, bytes4 functionSelector);
    error PermanentlyOn();
    error TimelockActive(address target, bytes4 functionSelector, uint256 unlocksAt);
    ...
    function checkTransaction(
        ...
        // Check user timelocks if registry is set
        _checkTimelock(to, functionSelector);
    }

    /// @dev Internal function to check timelocks, separated to avoid stack too deep
    function _checkTimelock(address to, bytes4 functionSelector) internal view {
        if (timelockRegistry != address(0)) {
            (bool locked, uint256 unlocksAt) =
                RumpelTimelockRegistry(timelockRegistry).isLocked(msg.sender, to, functionSelector);
            if (locked) {
                revert TimelockActive(to, functionSelector, unlocksAt);
            }
        }
    }
    ...
    /// @notice Set the timelock registry address.
    /// @dev Set to address(0) to disable timelock checks
    /// @param _registry The timelock registry contract address
    function setTimelockRegistry(address _registry) external onlyOwner {
        timelockRegistry = _registry;
        emit TimelockRegistrySet(_registry);
    }
}
