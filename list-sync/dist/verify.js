...
function normalizeSnapshotRecord(rec) {
    switch (rec.kind) {
        case 'guard.protocol':
            return {
                slug: rec.tag,
        ...
        default:
            throw new Error(`Unknown snapshot kind ${rec.kind}`);
    }
}
function canonicalize(record) {
    ...
    if (record.token)
        parts.push(record.token);
    if (record.selector)
        parts.push(record.selector);
    if (record.state)
        parts.push(record.state);
    if (record.transferState)
        parts.push(`transfer:${record.transferState}`);
    if (record.approveState)
        parts.push(`approve:${record.approveState}`);
    if (record.blockTransfer !== undefined)
...
export function verifySnapshotMatchesYaml() {
